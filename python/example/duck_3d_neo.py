...
from py_diff_pd.env.duck_env_3d_neo import DuckEnv3dNeo

if __name__ == '__main__':
    seed = 42
    np.random.seed(seed)

    folder = Path('duck_3d_final')
    ...
    # methods = ('pd_eigen_pcg_proj_abs', 'pd_eigen_pcg_proj_clmp')
    # opts = ({ 'max_pd_iter': 5000, 'max_ls_iter': 10, 'abs_tol': 1e-9, 'rel_tol': 1e-4, 'verbose': 0, 'thread_ct': thread_ct,
    #             'use_bfgs': 1, 'bfgs_history_size': 10, 'project_newton': 1, 'use_abs': 1 },
    #         { 'max_pd_iter': 5000, 'max_ls_iter': 10, 'abs_tol': 1e-9, 'rel_tol': 1e-4, 'verbose': 0, 'thread_ct': thread_ct,
    #             'use_bfgs': 1, 'bfgs_history_size': 10, 'project_newton': 1, 'use_abs': 0 },)
    ...
    for method, opt in zip(methods, opts):
        ...
        def loss_and_grad(x):
            ...
            print('method: '+method+', loss: {:8.3f}, |grad|: {:8.3f}, forward time: {:6.3f}s, backward time: {:6.3f}s'.format(
