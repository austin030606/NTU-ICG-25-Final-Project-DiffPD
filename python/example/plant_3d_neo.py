...
from py_diff_pd.env.plant_env_3d_neo import PlantEnv3dNeo

if __name__ == '__main__':
    seed = 42
    np.random.seed(seed)
    folder = Path('plant_3d_final')
    youngs_modulus = 1e6
    poissons_ratio = 0.4
    env = PlantEnv3dNeo(seed, folder, {
    ...
    E = np.exp(x_init[0])
    nu = np.exp(x_init[1])
    env_opt = PlantEnv3dNeo(seed, folder, { 'youngs_modulus': E, 'poissons_ratio': nu })
    ...
    for method, opt in zip(reversed(methods), reversed(opts)):
        ...
        def loss_and_grad(x):
            ...
            grad = grad * np.exp(x)
            print('method: '+method+', loss: {:8.3f}, |grad|: {:8.3f}, E: {:8.3e}, nu: {:4.3f}, forward time: {:6.3f}s, backward time: {:6.3f}s'.format(
